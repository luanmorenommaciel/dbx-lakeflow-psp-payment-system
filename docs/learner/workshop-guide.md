...
You will give Claude Code or Codex a payment specification, approve its plan, and progressively build every
product artifact: contracts, synthetic data, Bronze, DQX-backed Silver/quarantine, Gold, a dev bundle, and Genie
Code investigation assets. Each learner or pair deploys to its own Databricks Free Edition workspace.

The repository supplies the safe harness—agent authority, module prompts, tests, fallback data, and recovery
tags—not a product to copy. The flow is:

```text
payment specification → agent plan → human approval → implementation → checkpoint
                      → human deploy approval → DAB deploy → UC/DQX/Gold/Genie evidence
```
...
## 09:00–09:35 — Module 01: contract and plan

Input: [01-contract-plan.md](prompts/01-contract-plan.md).

You and the agent translate [the business specification](../specs/psp-payment.md) into the payment contract and
incident ledger. Review the proposed entities, currencies, counts, seven invalid conditions, and held valid
chargeback before approving implementation.
