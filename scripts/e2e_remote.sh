...
stale_evidence=(
  ...
  "$evidence_dir/13-reset.log"
  "$evidence_dir/14-restore-deploy.log"
  "$evidence_dir/15-restore-upload.log"
  "$evidence_dir/16-restore-baseline.log"
  "$evidence_dir/17-restore-assert.log"
  "$evidence_dir/18-restore-genie.log"
  "$evidence_dir/19-restore-genie-test.log"
...
if [[ "$cleanup_mode" == "--reset-and-restore" ]]; then
  ...
  ./scripts/reset.sh --confirm "${WORKSHOP_SCHEMA:-${BUNDLE_VAR_schema:-dbx_agentic_dev}}" \
    | tee "$evidence_dir/13-reset.log"
  stage="restore-deploy"
  databricks bundle deploy -t dev -p "$profile" \
    | tee "$evidence_dir/14-restore-deploy.log"
  stage="restore-upload"
  ./scripts/upload_fallback.sh baseline | tee "$evidence_dir/15-restore-upload.log"
  stage="restore-run"
  databricks bundle run workshop_pipeline -t dev -p "$profile" --full-refresh-all \
    | tee "$evidence_dir/16-restore-baseline.log"
  stage="restore-assert"
  ./scripts/verify_remote_data.sh baseline | tee "$evidence_dir/17-restore-assert.log"
  DATABRICKS_GENIE_SPACE_ID="$(<"$evidence_dir/genie-space-id.txt")"
  export DATABRICKS_GENIE_SPACE_ID
  stage="restore-genie"
  ./scripts/create_genie_space.sh | tee "$evidence_dir/18-restore-genie.log"
  stage="restore-genie-test"
  ./scripts/test_genie.sh | tee "$evidence_dir/19-restore-genie-test.log"
